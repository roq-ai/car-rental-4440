...
const SidebarContent = ({ onClose, ...rest }: SidebarProps) => {
  ...
  const tenantRoles = ['read'];
  const isTenantUser = tenantRoles.some((role) => session?.user?.roles?.includes(role));
  const { data, error, isLoading } = useSWR<GuestInterface[]>(
    () => (isTenantUser && session?.user?.tenantId ? `/guests` : null),
    () => getGuests({ tenant_id: session?.user?.tenantId }).then(({ data }) => data),
  );
  loading = isLoading;
  ...
  const MockedLinkItems: Array<NavItemPropsInterface> = [
    ...
    {
      name: 'Guests',
      path: isTenantUser ? `/guests/view/${data?.[0]?.id}` : '/guests',
      entity: 'guest',
